...
// 问题(1): 一字排开报奇数剔除
// J(1)=1, J(2k)=2J(k), J(2k+1)=2J(k)
long long J1(long long n) {
    // 闭形式: J(n) = 2^p，其中2^p是<=n的最大2的幂
    long long p = 1;
    while (p * 2 <= n) p *= 2;
    return p;
}
...
// 问题(2): 环形报数，逢m的倍数剔除
// J(1)=1, J(n)=(J(n-1)+m-1)%n+1
long long J2(long long n, long long m) {
    long long ans = 1;
...
<left><img src="pic/17965_1414478251872_7919.jpg"></left>
...
因此，此题尽量避免用递归过程来实现（特别是第（2）问），而要想方设法推导出"递推公式"，由递推公式直接计算得到。
本题设置的时限很短，若用递归来实现的，都将返回超时或爆栈的运行错误。
...
运之星游戏。更一般的，由于人编号是从1开始而非0开始，所以第一个剔除的人编号为 (m-1)%n+1，
