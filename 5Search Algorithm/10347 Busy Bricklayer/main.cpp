...
struct Job { int s, d, p; }; // 起始时间、工期、每天工钱
Job j[105];
int n, best;

// 按起始时间排序便于相容判断
bool cmp(Job& a, Job& b) { return a.s < b.s; }

void dfs(int i, int cur, int lastEnd) {
    if (i > n) { best = max(best, cur); return; }
    // 上界剪枝
    int bound = cur;
    for (int k = i; k <= n; k++) bound += j[k].p * j[k].d;
    ...
    // 选（需相容）
    if (j[i].s > lastEnd)
        dfs(i + 1, cur + j[i].p * j[i].d, j[i].s + j[i].d - 1);
}

int main() {
    cin >> n;
    for (int i = 1; i <= n; i++) cin >> j[i].s;
