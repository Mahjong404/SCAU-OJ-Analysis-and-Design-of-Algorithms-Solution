...
// 归并a[low..mid]和a[mid+1..high]，统计逆序对并排序
void mergeCount(int low, int mid, int high) {
    int i = low, j = mid + 1, k = low;
    while (i <= mid && j <= high) {
        if (a[i] <= a[j])
            tmp[k++] = a[i++];
        else {
            // 左段a[i..mid]都大于a[j]，全部构成逆序
            inv += mid - i + 1;
            tmp[k++] = a[j++];
        }
    }
...
void countInversion(int low, int high) {
    if (low >= high) return;
    int mid = low + (high - low) / 2;
    countInversion(low, mid);       // 左段逆序
    countInversion(mid + 1, high);  // 右段逆序
    mergeCount(low, mid, high);     // 跨段逆序+归并排序
}
