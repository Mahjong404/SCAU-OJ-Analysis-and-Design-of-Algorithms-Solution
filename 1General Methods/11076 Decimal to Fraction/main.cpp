...
int main() {
    ...
    if (l == -1) {  // 有限小数
        ...
        a = s.substr(2, l - 2);          // 非循环部分
        b = s.substr(l + 1, r - l - 1);  // 循环部分
        ...
        // pow_n = 10^非循环位数, pow_m = 10^循环位数
        long long pow_n = 1;
        for (int i = 0; i < (int)a.size(); i++) pow_n *= 10;
        long long pow_m = 1;
        for (int i = 0; i < (int)b.size(); i++) pow_m *= 10;

        // 循环小数转分数公式
        long long numerator = A * (pow_m - 1) + B;
        long long denominator = (pow_m - 1) * pow_n;
