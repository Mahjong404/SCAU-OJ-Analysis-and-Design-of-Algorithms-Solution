...
int quickSelect(int l, int r, int k) {
    ...
    return k; // k落在[j+1, i-1]即等于pivot的区域
}

// 分治求众数：区间[l..r]
void findMode(int l, int r) {
    if (l > r) return;
    if (r - l + 1 <= bestCnt) return; // 剪枝

    // 找中位数
    int midPos = quickSelect(l, r, l + (r - l) / 2);
    int median = a[midPos];

    // 三向划分：[l..lt-1] < median, [lt..gt] == median, [gt+1..r] > median
    int lt = l, gt = r, cur = l;
    while (cur <= gt) {
        if (a[cur] < median) swap(a[lt++], a[cur++]);
        else if (a[cur] > median) swap(a[gt--], a[cur]);
        else cur++;
    }
    int cnt = gt - lt + 1; // 中位数的个数

    // 更新最优解
    if (cnt > bestCnt || (cnt == bestCnt && median < bestVal)) {
        bestCnt = cnt;
        bestVal = median;
    }

    // 递归：哪边更长就搜哪边（两边都可能搜）
    if (lt - l > bestCnt) findMode(l, lt - 1); // 左边长度 > 当前最优才搜
    if (r - gt > bestCnt) findMode(gt + 1, r); // 右边长度 > 当前最优才搜
}
